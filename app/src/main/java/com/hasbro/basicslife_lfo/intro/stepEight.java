...
import com.hasbro.basicslife_lfo.databinding.StepeightBinding;
...
public class stepEight extends AppCompatActivity {
    ...
    private StepeightBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = StepeightBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        initializeIntentData();
        setupDynamicMonthLabels();
    ...
    private void setupDynamicMonthLabels() {
        SimpleDateFormat monthFormat = new SimpleDateFormat("MMM yyyy", Locale.ENGLISH);
        SimpleDateFormat dateFormat = new SimpleDateFormat("dd MMM yyyy", Locale.ENGLISH);
        Calendar calendar = Calendar.getInstance();

        // Update current date
        String todayLabel = "SALES UPDATED AS OF - " + dateFormat.format(calendar.getTime());
        binding.txtSalesUpdated.setText(todayLabel);

        // Set last 3 months
        calendar.add(Calendar.MONTH, -1);
        binding.txtMonth1.setText(monthFormat.format(calendar.getTime()));

        calendar.add(Calendar.MONTH, -1);
        binding.txtMonth2.setText(monthFormat.format(calendar.getTime()));

        calendar.add(Calendar.MONTH, -1);
        binding.txtMonth3.setText(monthFormat.format(calendar.getTime()));
    }
    ...
    private void goToStepNine() {
        Intent intent = new Intent(stepEight.this, stepNine.class);
